...
public class StatSystemTests
{
	...
	{
		StatModifier modifier = new StatModifier("test", OperationType.Custom, 0, 0, (baseValue, currentValue) =>
		{
			return baseValue / 2f;
		});
	...
	{
		StatBaseValueChangeHandler testHandler = new StatBaseValueChangeHandler(STAT_NAME, (targetState, targetValue, container) =>
		{
			return Mathf.Max(targetValue, 0);
		});
